...
#define INPUT "Max.asm"
#define OUTPUT "Max.hack"
...
struct SymbolTable {
	Row *row;
	int numberOfRow;
	int variableIndexValid;
};

void initST(SymbolTable &table) {
	table.row = new Row[23];
	table.numberOfRow = 23;
	table.variableIndexValid = 23;
...
void addLabel(SymbolTable &table, string label, int line_index) {
	...
	++table.variableIndexValid;
}

void addLabel(SymbolTable &table, string label) {
	table.row = new Row[++table.numberOfRow];
	table.row[table.numberOfRow - 1].symbolName = label;
	table.row[table.numberOfRow - 1].value = table.variableIndexValid;
	++table.variableIndexValid;
}
...
void modify(string &line_data, SymbolTable table) {
	...
	if (index >= 0) value = to_string(table.row[index].value);
		...
		value = to_string(table.variableIndexValid - 1);
	}
	line_data = "@" + value;
}
...
void IOProcessing() {
			...
			for (unsigned int i = 0; i < lineLength; ++i) {
				if (infile_line_data[i] == ' ') {
					if (i == 0) {
						...
						infile_line_data = infile_line_data.substr(0, i - 1);
						lineLength = i;
						break;
					}
				}
	...
	while (getline(infile, infile_line_data)) {
		...
		//if there is no empty line, process
		if (lineLength) InstructionPrint(outfile, infile_line_data, table);
	}
